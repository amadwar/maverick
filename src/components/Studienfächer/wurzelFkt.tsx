    ...
    return (
        <div className="lernpfad-scrum">
          {data.length ? (  // überprüft, ob Daten vorhanden sind
            <>
              <span className="scrum">{data[22]["https://schema.org/termCode"]?.map(title => title["@value"]).join(', ')}</span>
          ...
          <div className="videos">
            <div><Link className="link" to="/video_wurzelFkt1"><img className="img" src={data[21]["https://schema.org/thumbnailUrl"]?.map(thumbnail => thumbnail["@value"]).join(', ')} alt="Thumbnail" /></Link><h6 style={{whiteSpace: "pre-line"}}>{data[21]["https://schema.org/title"]?.map(title => title["@value"]).join(', ')}</h6> </div>
            <div><Link className="link" to="/video_wurzelFkt2"><img className="img" src={data[23]["https://schema.org/thumbnailUrl"]?.map(thumbnail => thumbnail["@value"]).join(', ')} alt="Thumbnail" /></Link><h6 style={{whiteSpace: "pre-line"}}>{data[23]["https://schema.org/title"]?.map(title => title["@value"]).join(', ')}</h6>  </div>
            <div><Link className="link" to="/video_wurzelFkt3"><img className="img" src={data[24]["https://schema.org/thumbnailUrl"]?.map(thumbnail => thumbnail["@value"]).join(', ')} alt="Thumbnail" /></Link><h6 style={{whiteSpace: "pre-line"}}>{data[24]["https://schema.org/title"]?.map(title => title["@value"]).join(', ')}</h6>  </div>
            <div><Link className="link" to="/video_wurzelFkt4"><img className="img" src={data[25]["https://schema.org/thumbnailUrl"]?.map(thumbnail => thumbnail["@value"]).join(', ')} alt="Thumbnail" /></Link><h6 style={{whiteSpace: "pre-line"}}>{data[25]["https://schema.org/title"]?.map(title => title["@value"]).join(', ')}</h6>  </div>
            <div><Link className="link" to="/video_wurzelFkt5"><img className="img" src={data[26]["https://schema.org/thumbnailUrl"]?.map(thumbnail => thumbnail["@value"]).join(', ')} alt="Thumbnail" /></Link><h6 style={{whiteSpace: "pre-line"}}>{data[26]["https://schema.org/title"]?.map(title => title["@value"]).join(', ')}</h6>  </div>
          </div>
          <p>Article</p>
          <div className="videos">
          
          <a href={data[27]["https://schema.org/url"]?.map(url => url["@value"]).join(', ')} >{data[27]["https://schema.org/title"]?.map(title => title["@value"]).join(', ')}</a>
